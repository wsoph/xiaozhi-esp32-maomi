...
#include <cstring>
#include <new>
#include "sound_playback_limits.h"
...
void AudioService::Initialize(AudioCodec* codec) {
    ...
    audio_engine_->OnWakeWordDetected([this](const std::string& wake_word) {
        ...
        if (discard_voice_upload_on_wake_.load(std::memory_order_acquire)) {
            // Establish the upload cut-off in the detecting task. The application
            // event may wait behind a send-queue snapshot on the main task.
            DiscardVoiceUploadBacklog();
        }
...
void AudioService::AudioOutputTask() {
    while (true) {
        ...
        const uint32_t finished_playback_id = task->playback_end ? task->playback_id : 0;
        ...
        if (finished_playback_id != 0 && callbacks_.on_playback_finished) {
            callbacks_.on_playback_finished(finished_playback_id);
        }
    }
...
void AudioService::OpusCodecTask() {
    while (true) {
        ...
        if (!audio_decode_queue_.empty() &&
            ...
            task->playback_end = packet->playback_end;
                ...
                if (ret == ESP_AUDIO_ERR_OK) {
                    ...
                    if (task->type == kAudioTaskTypeEncodeToSendQueue) {
                        bool queued = false;
                        {
                            ...
                            if (voice_upload_gate_.CanQueue(task->voice_upload_generation)) {
...
void AudioService::PushTaskToEncodeQueue(AudioTaskType type, std::vector<int16_t>&& pcm) {
    ...
    uint32_t dropped_total = 0;
    {
        ...
        if (type == kAudioTaskTypeEncodeToSendQueue &&
            !(xEventGroupGetBits(event_group_) & AS_EVENT_AUDIO_PROCESSOR_RUNNING)) {
            return;
        }
        task->voice_upload_generation = voice_upload_gate_.CaptureGeneration();
...
VoiceUploadGate::Lease AudioService::AcquireVoiceUploadLease(uint32_t generation) {
    return voice_upload_gate_.TryAcquire(generation);
}

void AudioService::ResumeVoiceUpload() { voice_upload_gate_.Resume(); }

void AudioService::SetDiscardVoiceUploadOnWake(bool enabled) {
    discard_voice_upload_on_wake_.store(enabled, std::memory_order_release);
}

void AudioService::DiscardVoiceUploadBacklog() {
    // Suspend first so no packet can obtain a new send authorization while the
    // queued and in-progress encoding work is invalidated.
    voice_upload_gate_.Suspend();
    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
...
bool AudioService::PlaySound(const std::string_view& ogg) {
    if (!codec_->output_enabled()) {
        esp_timer_stop(audio_power_timer_);
        esp_timer_start_periodic(audio_power_timer_, AUDIO_POWER_CHECK_INTERVAL_MS * 1000);
        codec_->EnableOutput(true);
    ...
    try {
        OggDemuxer demuxer;
        demuxer.OnPacket(
            [&](const uint8_t* data, int sample_rate, int frame_duration_ms, size_t packet_size) {
                if (!queued) {
                    return;
                }
                auto packet = std::make_unique<AudioStreamPacket>();
                ...
                queued = PushPacketToDecodeQueue(std::move(packet), true);
            });
        if (demuxer.Process(buf, size) != size || !demuxer.Finish() || !queued) {
            ResetDecoder();
            ESP_LOGE(TAG, "Refusing invalid, incomplete, or unqueueable Ogg Opus sound");
            return false;
        }
    } catch (const std::bad_alloc&) {
        ResetDecoder();
        ESP_LOGE(TAG, "Not enough memory to stream Ogg Opus sound");
        return false;
    }
    return true;
}

bool AudioService::TryPlaySound(const std::string_view& ogg, bool wait, uint32_t playback_id) {
    const auto* buf = reinterpret_cast<const uint8_t*>(ogg.data());
    const size_t size = ogg.size();

    if (!wait && !NonBlockingSoundLimits::AcceptsContainerSize(size)) {
        ESP_LOGE(TAG, "Refusing empty or oversized nonblocking Ogg container");
        return false;
    }

    if (wait) {
        size_t packet_count = 0;
        OggDemuxer validator;
        validator.OnPacket([&](const uint8_t*, int, int, size_t) { ++packet_count; });
        if (validator.Process(buf, size) != size || !validator.Finish()) {
            ESP_LOGE(TAG, "Refusing invalid or incomplete Ogg Opus sound");
            return false;
        }
        ...
        try {
            OggDemuxer demuxer;
            demuxer.OnPacket([&](const uint8_t* data, int sample_rate, int frame_duration_ms,
                                 size_t packet_size) {
                if (!queued) {
                    return;
                }
                ...
                packet->frame_duration = frame_duration_ms;
                packet->payload.assign(data, data + packet_size);
                if (--packet_count == 0 && playback_id != 0) {
                    packet->playback_id = playback_id;
                    packet->playback_end = true;
                }
                queued = PushPacketToDecodeQueue(std::move(packet), true);
            });
            if (demuxer.Process(buf, size) != size || !demuxer.Finish() || !queued) {
            ...
            ResetDecoder();
            ESP_LOGE(TAG, "Not enough memory to queue Ogg Opus sound");
            return false;
        }
        return true;
    }

    std::vector<std::unique_ptr<AudioStreamPacket>> pending_packets;
    bool within_bounds = true;
    NonBlockingSoundBudget sound_budget;
    try {
        OggDemuxer demuxer;
        demuxer.OnPacket([&](const uint8_t* data, int sample_rate, int frame_duration_ms,
                             size_t packet_size) {
            ...
            if (!sound_budget.TryAddPacket(packet_size, static_cast<uint32_t>(frame_duration_ms))) {
                within_bounds = false;
                return;
            }
            auto packet = std::make_unique<AudioStreamPacket>();
        ...
        if (demuxer.Process(buf, size) != size || !demuxer.Finish() || !within_bounds ||
            pending_packets.empty()) {
            ESP_LOGE(TAG, "Refusing invalid, incomplete, or oversized Ogg Opus sound");
            return false;
        }
    } catch (const std::bad_alloc&) {
        ESP_LOGE(TAG, "Not enough memory to prepare Ogg Opus sound");
        return false;
    }

    if (playback_id != 0) {
        pending_packets.back()->playback_id = playback_id;
        pending_packets.back()->playback_end = true;
    }
    ...
    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
    if (service_stopped_.load() ||
        audio_decode_queue_.size() > NonBlockingSoundLimits::kMaxPackets ||
        pending_packets.size() > NonBlockingSoundLimits::kMaxPackets - audio_decode_queue_.size()) {
        return false;
    }
    ...
    try {
        ...
        while (appended > 0) {
            --appended;
            audio_decode_queue_.pop_back();
        }
        ESP_LOGE(TAG, "Not enough memory to queue nonblocking Ogg Opus sound");
        return false;
    }
    playback_drained_notified_ = false;
    audio_queue_cv_.notify_all();
